        ...
        return (
            <div>
                <nav className='my-navbar'>
                    <div className='inner'><a href='/adm'>Acessar painel</a></div>
                ...
                >
                    ...
                    {this.state.pontos.map(ponto => {
                        return <Marker latitude={ponto.latitude} longitude={ponto.longitude} offsetLeft={-20} offsetTop={-10}>
                            ...
                            <div >
                                <img onMouseEnter={() => this.toggle(ponto)} onMouseLeave={() => this.toggle('asfasd')} src={marker} width={25} height={25} title={ponto.local} alt='Ponto' />
                            </div>
                        </Marker>
                    })}
                        ...
                        <div className='card-pontos'>
                            ...
                            <Fade in={true}>
                                <Card>
                                    ...
                                    <CardImg top width='100%' src={(ponto.url) ? ponto.url : pontoImage} alt='Card image cap' />

                                    <CardBody>
                                        <CardTitle>{ponto.local}</CardTitle>

                                        <CardText><section>
                                            <div>{ponto.inicio} ás {ponto.termino}</div>
                                            <div>
                                            
                                            {

                                                ponto.dias ? (Object.keys(ponto.dias).map(key => {
                                                    return <div style={{display: 'inline-block'}} key={key}><div  style={{marginLeft: '3px'}} key={key}>{ponto.dias[key].label}, </div></div>
                                                })) : ('') 
                                            }
                                                </div>
